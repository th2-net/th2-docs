...
*Messages* are encoded and decoded by th2 components according to the protocol used and the information in the `messageMetaData` and the `fields` should be set up according to the specification of the protocol.
...
Within the th2 cluster, *messages* are transferred between th2 components by the message broker RabbitMQ. 
