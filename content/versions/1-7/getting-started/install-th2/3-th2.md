...
* Create directory on th2 node (the following command can require root permissions):
...
Keys uploaded before this date continue to work.
...
If required, add the `rabbitmq.ingress.hostName` value into [service.values.yaml](https://github.com/th2-net/th2-infra/blob/v1.7.3/example-values/service.values.yaml) file. Otherwise, the RabbitMQ HTTP service will be available on a node IP address.

### Define th2 Ingress parameters
Add `ingress.hostname` value if required into [service.values.yaml](https://github.com/th2-net/th2-infra/blob/v1.7.3/example-values/service.values.yaml) file. Otherwise, th2 HTTP services will be available on a node IP address.
...
# set credentials for existing Cassandra cluster
  dbUser:
    ...
    password: <password>
rabbitmq:
# set admin user credentials, it will be created during the deployment
