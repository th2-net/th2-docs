...
Please note that in the document *infra-schema* namespace is a name for any custom infra-schema created by users.
...
You should add PersistentVolume mapped to `/opt/components` directory and then create PersistentVolumeClaim once an *infra-schema* namespace is installed. 
...
Wait for all the pods in *service* namespace are up and running, once completed proceed with [infra-schema configuration](https://github.com/th2-net/th2-infra-schema-demo/blob/master/README.md) to deploy th2 namespaces.
